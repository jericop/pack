...
- **MVP note — assembly still uses the frontend.** The current build still uses `buildkit/cnbfrontend` to assemble the layers `FROM run-image` (per-layer `tar -xf`) and attach the build-metadata label; the retired part is the per-layer metadata-SHA REWRITE (now finalize authoring) and the `io.buildpacks.native.layer-order` label (superseded by `io.buildpacks.buildkit.native.build-metadata`). Known MVP limitation: assembly assumes the run image has sh/tar; the fix is to run `tar` from the BUILD image (run image never modified).
...
- **jericop/cnb-lifecycle** (`buildkit-native-export` branch): Exporter emit-mode computing the plan + build-metadata label (`phase/emit`) and the FINALIZE library authoring CNB metadata from produced diffIDs (`phase/finalize`), consumed by pack like `phase.Rebaser`. `buildkit/cnbfrontend` still assembles layers in the MVP. Published image `jericop/lifecycle:buildkit-native-export`. Pack consumes it as a library via a `replace` directive.
- **jericop/cnb-pack** (`buildkit-native-export` branch): The `buildkit-native` backend calling `finalize.Finalize` post-push (`backend_native.go`, `executor.go` single-arch routing, `metadata_rewrite.go` test-env shim only, `emit_contract.go`).
